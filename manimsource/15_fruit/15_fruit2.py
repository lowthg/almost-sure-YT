...
class CubicRearrange(SimplePoints):
    def construct(self):
        MathTex.set_default(stroke_width=2, font_size=62)

        eq1 = MathTex(r'u_1u_2(u_1+u_2)', r'+', r'u_0u_2(u_0+u_2)', r'+', r'u_0u_1(u_0+u_1)', r'-', r'11u_0u_1u_2')
        eq1[5:].next_to(eq1[:5], DOWN, buff=0.3).shift(RIGHT*0.5)
        eq1.move_to(ORIGIN)
        # eq2 = MathTex(r'u_1^2u_2 + u_1u_2^2', r'+', r'u_0^2u_2+u_0u_2^2', r'+', r'u_0^2u_1+u_0u_1^2', r'-', r'11u_0u_1u_2')
        eq2 = MathTex(r'u_2u_1^2 + u_1u_2^2', r'+', r'u_2u_0^2+u_0u_2^2', r'+', r'u_1u_0^2+u_0u_1^2', r'-', r'11u_0u_1u_2')
        mh.align_sub(eq2, eq2[3], eq1[3])
        mh.align_sub(eq2[5:], eq2[5], eq1[5])
        eq2[:5].move_to(ORIGIN, coor_mask=RIGHT)
        # mh.align_sub(eq3, eq3[3], eq2[3])
        # mh.align_sub(eq3[5:], eq3[5], eq2[5])
        eq3 = MathTex(r'\sum_{i\not=j}', r'u_iu_j^2', r'-', r'11u_0u_1u_2')
        mh.align_sub(eq3, eq3[2], eq2[3], coor_mask=UP)

        # VGroup(eq1[0][0], eq1[0][5], eq1[4][2])
        mh.rtransform.copy_colors = True
        VGroup(eq1[0][0], eq1[0][2], eq1[0][5], eq1[0][8], eq1[6][2::2],
               eq3[1][1], eq3[1][4], eq3[0][1], eq3[0][4]).set_color(col_x)
        VGroup(eq1[0][1], eq1[0][3], eq1[0][6], eq1[0][9], eq1[6][:2], eq1[6][3::2],
               eq2[0][3], eq2[0][-2]).set_color(col_num)
        VGroup(eq3[0][0], eq3[0][2:4]).set_color(col_op)
        mh.copy_colors_eq(eq1[0], eq1[2])
        mh.copy_colors_eq(eq1[0], eq1[4])
        mh.copy_colors_eq(eq2[0], eq2[2])
        mh.copy_colors_eq(eq2[0], eq2[4])

        eq1 = eq_shadow(eq1, bg_stroke_width=17)
        eq2 = eq_shadow(eq2, bg_stroke_width=17)
        eq3 = eq_shadow(eq3, bg_stroke_width=17)

        self.add(eq1)
        anims = []
        for i in [0,2,4]:
            shift1 = mh.diff(eq1[i][5], eq2[i][2], coor_mask=RIGHT)
            shift2 = mh.diff(eq1[i][-3], eq2[i][-3], coor_mask=RIGHT)
            anims += [Succession(Wait(0.5), AnimationGroup(mh.rtransform(eq1[i][0], eq2[i][2], eq1[i][1], eq2[i][4],
                                                                         eq1[i][2:4], eq2[i][:2],
                      eq1[i][7], eq2[i][5], eq1[i][:3].copy(), eq2[i][6:9], eq1[i][3].copy(), eq2[i][10],
                                    ),
                      mh.rtransform(eq1[i][5], eq2[i][2], eq1[i][6], eq2[i][4], eq1[i][8], eq2[i][8], eq1[i][9], eq2[i][10]),
                      eq2[i][3].set_opacity(-1).shift(-shift1).animate.shift(shift1).set_opacity(1),
                      eq2[i][-2].set_opacity(-1).shift(-shift2).animate.shift(shift2).set_opacity(1),
                                     run_time=2)),
                      FadeOut(eq1[i][4]),
                      FadeOut(eq1[i][-1]),
                      ]

        self.play(*anims, mh.rtransform(eq1[1::2], eq2[1::2], eq1[6], eq2[6], run_time=1.8))

        anims = []
        eqs = []
        for i in [0,2,4]:
            eqs += [eq3[1][4].copy(), eq3[1][1].copy(), eq3[1][4].copy(), eq3[1][1].copy()]
            anims += [AnimationGroup(mh.rtransform(eq2[i][0], eq3[1][0], eq2[i][2:4], eq3[1][2:4]),
                                     mh.rtransform(eq2[i][6], eq3[1][0], eq2[i][8:10], eq3[1][2:4]),
                                     mh.fade_replace(eq2[i][1], eqs[-1]),
                                     mh.fade_replace(eq2[i][4], eqs[-2]),
                                     mh.fade_replace(eq2[i][7], eqs[-3]),
                                     mh.fade_replace(eq2[i][10], eqs[-4]),
                                     run_time=2),
                          FadeOut(eq2[i][5]),
                      ]

        self.play(*anims, FadeOut(eq2[1], eq2[3]),
                  Succession(Wait(1), FadeIn(eq3[0])),
                  Succession(Wait(1), mh.rtransform(eq2[5:], eq3[2:], run_time=1.5)))
        self.remove(*eqs)
        self.add(eq3[1][4], eq3[1][1])
        self.wait()

class Mordell(SimplePoints):
    bgcolor = GREY
    trcolor = BLACK

    def construct(self):
        MathTex.set_default(font_size = 60, stroke_width=1.5, color=col_op * 0.5 + WHITE*0.5)
        eq1 = Tex(r"\underline{\sf Mordell's Theorem}", font_size=65, stroke_width=2, color=BLUE)

        # eq2 = Tex(r'\sf The rational points of an elliptic curve')
        # eq3 = Tex(r'\sf are finitely generated by the')
        # eq4 = Tex(r'\sf chord and tangent construction.')
        # gp = VGroup(eq1, eq2, eq3, eq4).set_z_index(1)
        # eq2.next_to(eq1, DOWN, buff=0.4)
        # eq3.next_to(eq2, DOWN)
        # shift = eq3[0][2].get_bottom() - eq2[0][3].get_bottom()
        # mh.align_sub(eq4, eq4[0][12], eq3[0][2], coor_mask=UP).shift(shift*UP)

        eq2 = Tex(r'\sf The rational points of an elliptic curve are finitely ')
        eq3 = Tex(r'\sf generated by the chord and tangent construction.')
        gp = VGroup(eq1, eq2, eq3).set_z_index(1)
        eq2.next_to(eq1, DOWN, buff=0.4)
        eq3.next_to(eq2, DOWN)

        box = SurroundingRectangle(gp, fill_color=BLACK, fill_opacity=0.7, stroke_opacity=0, stroke_width=0,
