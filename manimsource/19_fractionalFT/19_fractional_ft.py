...
class RotateHarmonic(STFTWindow):
    bgcol = GREY
    trcol = BLACK
    def construct(self):
        self.do_anim()
    ...
    def do_anim(self, just_eq=False):
        ...
        VGroup(eq1[0][4], eq1[2][0]).set_color(col_x)
        VGroup(eq2[0][4], eq1[2][6]).set_color(col_p)
        ...
        VGroup(eq7[0][0], eq7[0][-2], eq8[0][0], eq8[0][-2]).set_color(col_op)
        VGroup(eq9[2][-1], eq10[2][-3], eq11[0], eq11[2]).set_color(RED)
        VGroup(eq11[4]).set_color(col_num)
...
class RotateHarmonic2(RotateHarmonic):
    ...
    def construct(self):
        ...
        arr1 = Arrow(pt1, pt1 + (dir1[0]*DOWN+dir1[1]*RIGHT)*0.3, color=RED, stroke_width=7, buff=0,
                     max_stroke_width_to_length_ratio=20).set_z_index(2)


        dl = VGroup(dot1, line1)
        self.add(ax, line1, dot1, eq2, eq3, eq4, dot2)
        self.wait(0.1)
        dl2 = VGroup(dot1, line1, arr1)
