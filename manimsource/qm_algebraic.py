...
class Star(Scene):
    def construct(self):
        eq1 = Tex(r'\underline{*-algebra $\mathcal A$}', color=RED, font_size=60)[0]
        eq1[-1].set_color(WHITE)
        eq1[-1].next_to(eq1[2], DOWN, buff=0.04, coor_mask=UP)
        eq2 = Tex(r'$\mathcal A$ is a $\mathbb C$-vector space').next_to(eq1, DOWN, buff=0.4)
        eq3 = Tex(r'a(bc)=a(bc)')

        gp = VGroup(eq1, eq2).move_to(ORIGIN)

        self.add(gp)

class State(Pdefinition):
    def construct(self):
        MathTex.set_default(font_size=60)
        Tex.set_default(font_size=60)
        s = 0.4
        eq1 = Tex(r'\underline{States on *-algebra $\mathcal A$}', color=RED)[0].set_z_index(1)
        eq1[-1].set_color(WHITE)
        eq1[-1].next_to(eq1[1], DOWN, buff=0.04, coor_mask=UP).set_z_index(0.5)
        eq2 = Tex(r'$P\colon\mathcal A\to\mathbb C$').set_z_index(1).next_to(eq1, DOWN, buff=0.4*s)
        eq2_2 = Tex(r'linearity: $P(\lambda A+\mu B)=\lambda P(A)+\mu P(B)$').set_z_index(1).next_to(eq2, DOWN, buff=0.4*s)
        eq3 = Tex(r'total probability: $P(1)=1$').set_z_index(1).next_to(eq2_2, DOWN, buff=0.4*s)
        eq4 = Tex(r'positivity: $P(A^*A)\ge0$').set_z_index(1).next_to(eq3, DOWN, buff=0.4*s)

        gp = VGroup(eq1, eq2, eq2_2, eq3, eq4).move_to(ORIGIN)
        box1 = SurroundingRectangle(gp, fill_color=BLACK, stroke_opacity=0, fill_opacity=self.box_op, corner_radius=0.15)

        self.add(gp, box1)

class Density1(Scene):
    def construct(self):
        fs1 =100
        fs2 = 80
        eq1 = MathTex(r'\Psi', font_size=200)
        eq2 = MathTex(r'P(A)', r'=', r'\langle\Psi\vert A\vert\Psi\rangle', font_size=fs1)
        eq2.next_to(eq1, DOWN, buff=0.4)
        gp1 = VGroup(eq1.copy(), eq2).move_to(ORIGIN)
        self.add(eq1)
        self.wait(0.1)
        self.play(mh.transform(eq1, gp1[0]), FadeIn(eq2), run_time=1.4)
        self.wait(0.1)
        eq3 = MathTex(r'u_1,u_2,\ldots, u_n', font_size=fs1)
        eq3.next_to(eq2, DOWN, buff=0.6)
        gp2 = VGroup(eq2.copy(), eq3).move_to(ORIGIN)
        self.play(FadeOut(eq1, shift=mh.diff(eq2, gp2[0])),
                  mh.transform(eq2, gp2[0]),
                  FadeIn(eq3),
                  run_time=1.4)
        self.wait(0.1)

        eq4 = MathTex(r'\Psi', r'=', r'c_1u_1+c_2u_2+\cdots+c_nu_n', font_size=fs2)
        eq4.next_to(eq3, DOWN, buff=0.5)
        gp3 = VGroup(eq2.copy(), eq3.copy(), eq4).move_to(ORIGIN)
        self.play(mh.transform(eq2, gp3[0], eq3, gp3[1]),
                  FadeIn(eq4),
                  run_time=1.4)
        self.wait(0.1)

        eq5 = MathTex(r'\Psi', r'=', r'\langle u_1\vert\Psi\rangle u_1+\langle u_2\vert\Psi\rangle u_2+',
                                     r'\cdots+\langle u_n\vert\Psi\rangle u_n', font_size=fs2)
        eq5[3].next_to(eq5[2], DOWN, buff=0.4).align_to(eq5[2], RIGHT)
        mh.align_sub(eq5, eq5[1], eq4[1], coor_mask=UP).move_to(ORIGIN, coor_mask=RIGHT)

        self.play(mh.rtransform(eq4[:2], eq5[:2], eq4[2][2:5], eq5[2][6:9]),
                  mh.fade_replace(eq4[2][:2], eq5[2][:6], coor_mask=RIGHT),
                  eq4[2][5:10].animate.shift(mh.diff(eq4[2][5], eq5[2][9])),
                  eq4[2][10:].animate.shift(mh.diff(eq4[2][10], eq5[3][0])),
                  run_time=1.5)
        self.play(mh.rtransform(eq4[2][7:10], eq5[2][15:18]),
                  mh.fade_replace(eq4[2][5:7], eq5[2][9:15], coor_mask=RIGHT),
                  run_time=1.5)
        self.play(mh.rtransform(eq4[2][10:14], eq5[3][:4], eq4[2][16:18], eq5[3][10:12]),
                  mh.fade_replace(eq4[2][14:16], eq5[3][4:10], coor_mask=RIGHT),
                  run_time=1.5)
        self.wait(0.1)

        eq6 = MathTex(r'\Psi', r'=', r'\sum_j\langle u_j\vert\Psi\rangle u_j', font_size=fs2)
        mh.align_sub(eq6, eq6[1], eq5[1]).move_to(ORIGIN, coor_mask=RIGHT)
        gp4 = VGroup(eq6[2][4].copy(), eq6[2][9].copy(), eq6[2][4].copy(), eq6[2][9].copy())
        self.play(mh.rtransform(eq5[:2], eq6[:2], eq5[2][:2], eq6[2][2:4], eq5[2][3:7], eq6[2][5:9]),
                  mh.fade_replace(eq5[2][2], eq6[2][4]),
                  mh.fade_replace(eq5[2][7], eq6[2][9]),
                  mh.rtransform(eq5[2][9:11], eq6[2][2:4], eq5[2][12:16], eq6[2][5:9]),
                  mh.fade_replace(eq5[2][11], gp4[0]),
                  mh.fade_replace(eq5[2][16], gp4[1]),
                  mh.rtransform(eq5[3][4:7], eq6[2][2:4], eq5[3][7:11], eq6[2][5:9]),
                  mh.fade_replace(eq5[3][6], gp4[2]),
                  mh.fade_replace(eq5[3][11], gp4[3]),
                  FadeOut(eq5[2][8], eq5[2][17], eq5[3][:4]),
                  FadeIn(eq6[2][:2]),
                  run_time=1.8)
        gp4.set_opacity(0)
...
class Density(Scene):
    def construct(self):
        ...
        eq4 = MathTex(r'P(A)', r'=', r'\langle\Psi\vert A\vert\Psi\rangle', font_size=fs2)
        eq4.next_to(eq3, DOWN, buff=0.7)
        gp3 = VGroup(eq2.copy(), eq3.copy(), eq4).move_to(ORIGIN)
        self.play(mh.transform(eq2, gp3[0], eq3, gp3[1]),
                  mh.rtransform(eq2.copy(), eq4),
                  run_time=1.5)
        self.wait(0.1)

        eq5 = MathTex(r'P(A)', r'=', r'\langle\Psi\vert A\sum_k\vert u_k\rangle\langle u_k\vert\Psi\rangle', font_size=fs2)
        mh.align_sub(eq5, eq5[1], eq4[1], coor_mask=UP)
        self.play(mh.rtransform(eq4[:2], eq5[:2], eq4[2][:4], eq5[2][:4], eq4[2][4:], eq5[2][-3:]),
                  run_time=1.4)
        self.play(FadeIn(eq5[2][4:-3]), rate_func=linear, run_time=1.4)

        eq6 = mh.circle_eq(eq5[2][4:-2]).set_z_index(1)
        eq7 = Tex(r'identity', font_size=60).set_z_index(2)
        eq7.next_to(eq6, DR, buff=0).shift(UL*0.5)
        self.play(Create(eq6), FadeIn(eq7), run_time=1)
        self.wait(0.1)
        eq8 = MathTex(r'P(A)', r'=', r'\sum_k\langle\Psi\vert A\vert u_k\rangle\langle u_k\vert\Psi\rangle', font_size=fs2)
        mh.align_sub(eq8, eq8[1], eq5[1])
        self.play(mh.rtransform(eq5[:2], eq8[:2], eq5[2][:4], eq8[2][2:6], eq5[2][4:6], eq8[2][:2],
                                eq5[2][6:], eq8[2][6:]), FadeOut(eq6, eq7, rate_func=rush_into), run_time=1.2)
        self.wait(0.1)
        eq9 = MathTex(r'P(A)', r'=', r'\sum_k\langle u_k\vert\Psi\rangle\langle\Psi\vert A\vert u_k\rangle', font_size=fs2)
        mh.align_sub(eq9, eq9[1], eq8[1])
        self.play(mh.rtransform(eq8[:2], eq9[:2], eq8[2][:2], eq9[2][:2], eq8[2][2:10], eq9[2][8:16], eq8[2][10:16], eq9[2][2:8]),
                  run_time=1.5)

        self.wait(0.1)
        eq10 = MathTex(r'\rho', r'=', r'\vert\Psi\rangle\langle\Psi\vert', font_size=fs2)
        eq10.next_to(eq9, DOWN, buff=0.2)
        mh.align_sub(eq10, eq10[2], eq9[2][5:11], coor_mask=RIGHT)
        gp4 = VGroup(eq2.copy(), eq3.copy(), eq9.copy(), eq10).move_to(ORIGIN, coor_mask=UP).shift(UP*0.3)
        self.play(mh.transform(eq2, gp4[0], eq3, gp4[1], eq9, gp4[2]),
                  mh.rtransform(eq9[2][5:11].copy(), eq10[2][:]),
                  FadeIn(eq10[:2]),
                  run_time=2)
        self.wait(0.1)
        eq11 = MathTex(r'P(A)', r'=', r'\sum_k\langle u_k\vert\rho A\vert u_k\rangle', font_size=fs2)
        mh.align_sub(eq11, eq11[1], eq9[1], coor_mask=UP)
        eq11_1 = eq11[2][6].copy().move_to(eq9[2][6:11], coor_mask=RIGHT)
        self.play(FadeOut(eq9[2][6:11]), mh.rtransform(eq10[0].copy(), eq11_1), run_time=1.3)
        self.wait(0.1)
        self.play(mh.rtransform(eq9[:2], eq11[:2], eq9[2][:6], eq11[2][:6], eq11_1, eq11[2][6], eq9[2][11:], eq11[2][7:]),
                  eq10.animate.move_to(ORIGIN, coor_mask=RIGHT),
                  run_time=1.2)
        self.wait(0.1)
        eq12 = MathTex(r'P(A)', r'=', r'{\rm tr}(\rho A)', font_size=fs1)
        eq12_1 = eq12
        mh.align_sub(eq12, eq12[1], eq11[1], coor_mask=UP)
        self.play(FadeOut(eq11[2][:6], eq11[2][8:]),
                  FadeIn(eq12[2][:3], eq12[2][-1]),
                  run_time=1.8)
        self.play(mh.rtransform(eq11[:2], eq12[:2], eq11[2][6:8], eq12[2][3:5]),
                  run_time=1.2)
        self.wait(0.1)
        self.play(FadeOut(eq3), rate_func=linear)
        self.wait(0.1)

        eq12 = MathTex(r'P(A)', r'=', r'\sum_kp_k\langle\Psi_k\vert A\vert\Psi_k\rangle', font_size=fs1)
        mh.align_sub(eq12, eq12[1], eq2[1], coor_mask=UP)
        self.play(mh.rtransform(eq2[:2], eq12[:2], eq2[2][:2], eq12[2][4:6], eq2[2][2:6], eq12[2][7:11], eq2[2][6], eq12[2][12]),
                  FadeIn(eq12[2][6], shift=mh.diff(eq2[2][1], eq12[2][5])),
                  FadeIn(eq12[2][11], shift=mh.diff(eq2[2][5], eq12[2][10])),
                  FadeIn(eq12[2][:4], rate_func=rush_into),
                  run_time=1.8)
        eq13 = Tex(r'mixed state', font_size=fs2, color=RED)
        eq13.next_to(eq12[2][2:], DOWN, buff=0.4)
        self.play(FadeIn(eq13))
        self.wait(0.1)

        eq14 = MathTex(r'\rho', r'=', r'\sum_kp_k\vert\Psi_k\rangle\langle\Psi_k\rangle', font_size=fs1)
        mh.align_sub(eq14, eq14[1], eq10[1], coor_mask=UP)
        self.play(mh.rtransform(eq10[:2], eq14[:2], eq10[2][:2], eq14[2][4:6], eq10[2][2:5], eq14[2][7:10], eq10[2][5], eq14[2][11]),
                  FadeIn(eq14[2][6], shift=mh.diff(eq10[2][1], eq14[2][5])),
                  FadeIn(eq14[2][10], shift=mh.diff(eq10[2][4], eq14[2][9])),
                  FadeIn(eq14[2][:4], rate_func=rush_into),
                  run_time=2)
        self.wait(0.1)
        ...
        eq16 = MathTex(r'P(A)', r'=', r'{\rm tr}(\sqrt{\rho}A\sqrt{\rho})', font_size=fs1)
        mh.align_sub(eq16, eq16[1], eq15[1], coor_mask=UP)
        self.play(mh.rtransform(eq15[:2], eq16[:2], eq15[2][:3], eq16[2][:3], eq15[2][3], eq16[2][6],
                                eq15[2][4:6], eq16[2][9:11], eq15[2][4].copy(), eq16[2][5]),
...
def get_line(obj: Mobject):
    return Line(obj.get_corner(DL), obj.get_corner(UR), stroke_color=RED, stroke_width=8).set_z_index(2)
...
class DotProduct(Scene):
    def construct(self):
        ...
        fs2 = 70
        fs3 = 60
        eq1 = MathTex(r'{\rm tr}(\rho_1\rho_2)', font_size=fs1)[0]
        eq2 = MathTex(r'\frac14\!\begin{pmatrix}'
                             r'1+x_1 & y_1+iz_1 \\'
                             r'y_1-iz_1 & 1-x_1\end{pmatrix}',
                      r'\begin{pmatrix}'
                      r'1+x_2 & y_2+iz_2 \\'
                      r'y_2-iz_2 & 1-x_2\end{pmatrix}',
                      font_size=fs2).set_z_index(1)
        eq3 = MathTex(r'\frac14\begin{pmatrix}(1+x_1)(1+x_2) + (y_1+iz_1)(y_2-iz_2) \\'
                      r'+(y_1-iz_1)(y_2+iz_2) + (1-x_1)(1-x_2)\end{pmatrix}', font_size=fs3)[0]
        eq3[33:].shift(RIGHT*1.8)
        eq3.move_to(ORIGIN).next_to(eq2, DOWN, buff=0.6)
        self.add(eq1)
        self.wait(0.5)
        self.play(eq1.animate.to_edge(UP, buff=0.4), FadeIn(eq2), run_time=1.5)
        gp1 = VGroup(eq2.copy(), eq3).move_to(ORIGIN)
        self.wait(0.1)
        self.play(mh.transform(eq2, gp1[0]), FadeIn(eq3[:4], eq3[-1]), run_time=1.4)
        self.wait(0.1)
        eq4 = MathTex(r'(1+x_1)', r'(1+x_2)', r'(y_1+iz_1)', r'(y_2-iz_2)',
                      r'(y_1-iz_1)', r'(y_2+iz_2)', r'(1-x_1)', r'(1-x_2)', font_size=fs2).set_z_index(1)
        for _ in eq4[:]: VGroup(_[-1], _[0]).set_opacity(0)
        subeq = [eq2[0][4:8], eq2[1][1:5], eq2[0][8:14], eq2[1][11:17],
                 eq2[0][14:20], eq2[1][5:11], eq2[0][20:24], eq2[1][17:21]]
        for x,y in zip(eq4[:], subeq): mh.align_sub(x, x[1:-1], y)
        color = ManimColor(WHITE.to_rgb() * 0.5)
        buff = 0.07
        opacity = 0.5
        rec = [SurroundingRectangle(subeq[i] + subeq[j], stroke_opacity=0, fill_opacity=opacity, fill_color=color, buff=buff)
               for i, j in ((0, 2), (1, 3), (4, 6), (5, 7))]

        self.play(FadeIn(rec[0], rec[1]), run_time=1.5)

        self.play(mh.rtransform(eq4[0], eq3[4:10], eq4[1], eq3[10:16],
                                eq4[2], eq3[17:25], eq4[3], eq3[25:33]),
                  FadeIn(eq3[16], rate_func=rush_into),
                  run_time=2)
        self.wait(0.1)
        self.play(mh.rtransform(rec[0], rec[2], rec[1], rec[3]), run_time=1.5)
        self.play(mh.rtransform(eq4[4], eq3[34:42], eq4[5], eq3[42:50],
                                eq4[6], eq3[51:57], eq4[7], eq3[57:63]),
                  FadeIn(eq3[33]),
                  FadeIn(eq3[50], rate_func=rush_into),
                  FadeOut(*rec[2:], rate_func=linear),
                  run_time=2)

        self.wait(0.1)
        eq1_2 = MathTex(r'{\rm tr}(\rho_1\rho_2)=', font_size=fs1)[0].next_to(eq3, UP, coor_mask=UP, buff=0.7)
        gp2 = VGroup(eq1_2, eq3.copy()).move_to(ORIGIN, coor_mask=UP).shift(UP*0.6)
        self.play(FadeOut(eq2), mh.rtransform(eq1, eq1_2[:-1]),
                  FadeIn(eq1_2[-1], shift=mh.diff(eq1[0][-1], eq1_2[-2])),
                  mh.transform(eq3, gp2[1]),
                  run_time=1.5)

        eq5 = MathTex(r'1+x_1x_2+x_1+x_2 + (y_1+iz_1)(y_2-iz_2)', font_size=fs3)[0]
        eq6 = MathTex(r'+(y_1-iz_1)(y_2+iz_2)+1+x_1x_2-x_1-x_2', font_size=fs3)[0]
        mh.align_sub(eq5, eq5[0], eq3[5]).align_to(eq3[4], LEFT)
        mh.align_sub(eq6, eq6[16], eq3[50]).align_to(eq3[62], RIGHT)
        self.play(mh.rtransform(eq3[5:9], eq5[:4]),
                  mh.rtransform(eq3[11:13], eq5[:2], eq3[13:15], eq5[4:6],
                                eq3[6:9].copy(), eq5[6:9], eq3[12:15].copy(), eq5[9:12],
                                eq3[16:33], eq5[12:]),
                  FadeOut(eq3[4], eq3[9:11], eq3[15], rate_func=rush_from),
                  run_time=1.5)
        eq6_1 = eq6[19].copy()
        self.play(mh.rtransform(eq3[54:56] + eq3[52], eq6[20:22] + eq6[18]),
                  mh.rtransform(eq3[60:62], eq6[22:24], eq3[58], eq6[18],
                                eq3[53:56].copy(), eq6[24:27], eq3[59:62].copy(), eq6[27:30],
                                eq3[33:51], eq6[:18]),
                  FadeOut(eq3[51], eq3[56:58], eq3[62], rate_func=rush_from),
                  mh.fade_replace(eq3[53], eq6[19]),
                  mh.fade_replace(eq3[59], eq6_1),
                  run_time=1.5)
        eq6_1.set_opacity(0)
        line1 = get_line(eq5[6:12])
        line2 = get_line(eq6[24:])
        self.play(Create(line1), run_time=0.6)
        self.play(Create(line2), run_time=0.6)
        self.wait(0.1)
        self.play(FadeOut(line1, line2, eq5[6:12], eq6[24:]))
        self.wait(0.1)

        eq7 = MathTex(r'1+x_1x_2+y_1y_2+z_1z_2-iy_1z_2+iz_1y_2', font_size=fs3)[0]
        eq8 = MathTex(r'+y_1y_2+z_1z_2+iy_1z_2-iz_1y_2 + 1+x_1x_2', font_size=fs3)[0]
        mh.align_sub(eq7, eq7[0], eq5[0])
        #mh.align_sub(eq8, eq8[-6], eq6[-12]).align_to(eq6, RIGHT)
        mh.align_sub(eq8, eq8[0], eq6[0])#.align_to(eq6, RIGHT)
        self.play(mh.rtransform(eq5[12], eq7[6], eq5[14:16], eq7[7:9], eq5[22:24], eq7[9:11],
                                eq5[16], eq7[11], eq5[18:20], eq7[12:14], eq5[26:28], eq7[14:16]),
                  mh.rtransform(eq5[24:26], eq7[16:18], eq5[14:16].copy(), eq7[18:20],
                                eq5[26:28].copy(), eq7[20:22], eq5[16].copy(), eq7[22],
                                eq5[17], eq7[23], eq5[18:20].copy(), eq7[24:26],
                                eq5[22:24].copy(), eq7[26:28], eq5[:6], eq7[:6]),
                  FadeOut(eq5[13], eq5[20:22], eq5[28], rate_func=rush_from),
                  run_time=1.8)
        self.wait(0.1)
        self.play(mh.rtransform(eq6[17:24], eq8[-7:], eq6[0], eq8[0],
                                eq6[2:4], eq8[1:3], eq6[10:12], eq8[3:5], eq6[6:8], eq8[6:8],
                                eq6[14:16], eq8[8:10], eq6[12:14], eq8[10:12],
                                eq6[2:4].copy(), eq8[12:14], eq6[14:16].copy(), eq8[14:16],
                                eq6[4:6], eq8[16:18], eq6[6:8].copy(), eq8[18:20],
                                eq6[10:12].copy(), eq8[20:22]),
                  FadeOut(eq6[1], eq6[8:10], eq6[16], rate_func=rush_from),
                  mh.fade_replace(eq6[4].copy(), eq8[5]),
                  run_time=1.8)
        self.wait(0.1)
        line1 = get_line(eq7[-12:])
        line2 = get_line(eq8[10:22])
        self.play(Create(line1), run_time=0.6)
        self.play(Create(line2), run_time=0.6)
        self.wait(0.1)
        self.play(FadeOut(line1, line2, eq7[-12:], eq8[10:22]))
        self.wait(0.1)


        eq9 = MathTex(r'+1+x_1x_2+y_1y_2+z_1z_2', font_size=fs3)[0]
        mh.align_sub(eq9, eq9[0], eq8[0])
        self.play(mh.rtransform(eq8[-7:], eq9[:7], eq8[:10], eq9[-10:]),
                  run_time=1.8)
        self.wait(0.1)

        eq10 = MathTex(r'1+x_1x_2+y_1y_2+z_1z_2', font_size=fs2)[0]
        eq11 = MathTex(r'\frac12', font_size=fs3)[0]
        eq10.move_to(eq3[3:]).align_to(eq9[1], LEFT)
        mh.align_sub(eq11, eq11[1], eq3[1])
        self.play(mh.rtransform(eq9[1:], eq10), mh.rtransform(eq7[:16], eq10),
                  mh.rtransform(eq3[:2], eq11[:2]),
                  mh.fade_replace(eq3[2], eq11[2]),
                  FadeOut(eq9[0]),
                  run_time=1.5)
        self.wait(0.1)
        eq12 = MathTex(r'1+\begin{pmatrix}x_1 \\ y_1 \\ z_1\end{pmatrix}\cdot\begin{pmatrix}x_2 \\ y_2 \\ z_2\end{pmatrix}',
                       font_size=fs2)[0]
        eq12[12].scale(2)
        mh.align_sub(eq12, eq12[0], eq10[0])
        self.play(mh.rtransform(eq10[:2], eq12[:2], eq10[2:4], eq12[4:6], eq10[4:6], eq12[15:17],
                                eq10[7:9], eq12[6:8], eq10[9:11], eq12[17:19], eq10[12:14], eq12[8:10],
                                eq10[14:16], eq12[19:21]),
                  FadeOut(eq10[6], eq10[11], rate_func=rush_from),
                  FadeIn(eq12[2:4], eq12[10:15], eq12[-2:]),
                  run_time=1.5)
        self.wait(0.1)
        eq13 = MathTex(r'{\rm tr}(\rho_1\rho_2)=', r'\frac12(1+\cos\theta)', font_size=90)
        self.play(mh.rtransform(eq1_2, eq13[0], eq11, eq13[1][:3], eq12[:2], eq13[1][4:6]),
                  mh.fade_replace(eq12[2:], eq13[1][6:10]),
                  mh.stretch_replace(eq3[3], eq13[1][3]),
                  mh.stretch_replace(eq3[-1], eq13[1][-1]),
                  run_time=2)
        self.wait(0.1)
        eq14 = MathTex(r'{\rm tr}(\rho_1\rho_2)=', r'\cos^2\left(\frac\theta 2\right)', font_size=90)
        mh.align_sub(eq14, eq14[0][-1], eq13[0][-1], coor_mask=UP)
        self.play(mh.rtransform(eq13[0], eq14[0], eq13[1][6:9], eq14[1][:3], eq13[1][9], eq14[1][5]),
                  FadeOut(eq13[1][:6]),
                  FadeIn(eq14[1][3], shift=mh.diff(eq13[1][8], eq14[1][2])),
                  FadeIn(eq14[1][4], rate_func=rush_into),
                  mh.stretch_replace(eq13[1][-1], eq14[1][-1]),
                  FadeIn(eq14[1][6:8], shift=mh.diff(eq13[1][9], eq14[1][5])*RIGHT),
                  run_time=1.5)
